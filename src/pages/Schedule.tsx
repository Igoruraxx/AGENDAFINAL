...
import { Clock, ChevronLeft, ChevronRight, Grid3x3, Plus, X, List, CheckCircle2, MessageCircle, Dumbbell, CalendarDays, Trash2, FileText } from 'lucide-react';
...
const Schedule: React.FC = () => {
  ...
  const handleDeleteApt = async () => {
    ...
    try {
      ...
      console.error('[Schedule] Erro ao deletar:', err.message);
    }
    closeAptModal();
  };
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4 z-[100]" style={{background:'rgba(0,0,0,0.4)',backdropFilter:'blur(4px)'}}>
          <div className="w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl overflow-hidden" style={{background:'var(--n-0)',border:'1px solid var(--n-200)',boxShadow:'var(--sh-lg)'}}>
            <div className="px-5 pt-5 pb-4" style={{borderBottom:'1px solid var(--n-200)'}}>
              <div className="flex items-center justify-between">
                <h3 className="text-base font-extrabold" style={{color:'var(--n-900)'}}>Agendar Atendimento</h3>
                <button
                  onClick={() => setShowQuickAdd(false)}
                  className="p-2 rounded-lg hover:bg-black/5 transition-colors touch-manipulation"
                >
                  <X size={18} style={{color:'var(--n-400)'}} />
                </button>
              </div>
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-2">
                  <label className="block text-xs font-semibold" style={{color:'var(--n-600)'}}>Cliente cadastrado</label>
                  <select
                    value={quickAddStudentId}
                    onChange={(e) => { setQuickAddStudentId(e.target.value); setQuickAddMode('existing'); }}
                    disabled={quickAddMode === 'trial'}
                    className={`input-base w-full ${quickAddMode === 'trial' ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                ...
                <div className="space-y-2">
                  <label className="block text-xs font-semibold" style={{color:'var(--n-600)'}}>Experimental</label>
                  <div className="flex gap-2 items-center">
                    <input
                      type="checkbox"
                      checked={quickAddMode === 'trial'}
                      onChange={(e) => setQuickAddMode(e.target.checked ? 'trial' : 'existing')}
                      className="w-4 h-4"
                      style={{accentColor:'var(--accent)'}}
                    />
                    <span className="text-xs" style={{color:'var(--n-500)'}}>Agendar para convidado</span>
                  </div>
                  {quickAddMode === 'trial' && (
                    <input
                      type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{color:'var(--n-600)'}}>Horário</label>
                  <select
                    value={quickAddTime}
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{color:'var(--n-600)'}}>Duração</label>
                  <select
            ...
            <div className="px-5 pt-5 pb-4 flex-shrink-0" style={{borderBottom:'1px solid var(--n-200)'}}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-extrabold" style={{color:'var(--n-900)'}}>{selectedApt.studentName}</h3>
                ...
                <button onClick={closeAptModal} className="p-2 rounded-lg hover:bg-black/5 transition-colors touch-manipulation">
                  <X size={18} style={{color:'var(--n-400)'}} />
                </button>
              </div>
            </div>

            <div className="px-5 py-4 space-y-5 overflow-y-auto flex-1" style={{background:'var(--n-0)'}}>
              {/* Actions */}
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => saveAptChanges(!selectedApt.sessionDone)}
                  className="flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all touch-manipulation"
                ...
                >
                  ...
                  {selectedApt.sessionDone ? 'Concluída ✓' : 'Marcar como feita'}
                </button>
                <a
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <Dumbbell size={15} style={{color:'var(--n-500)'}} />
                  <span className="text-sm font-bold" style={{color:'var(--n-900)'}}>Grupamentos treinados</span>
                  {modalMuscles.length > 0 && (
                    <span className="ml-auto text-xs font-semibold" style={{color:'var(--accent)'}}>{modalMuscles.length} selecionado{modalMuscles.length > 1 ? 's' : ''}</span>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-2">
                  {MUSCLE_GROUPS.map(mg => {
                    ...
                    return (
                      <button
                        key={mg.id}
                        onClick={() => { toggleMuscle(mg.id); haptic(6); }}
                        className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all touch-manipulation active:scale-95"
                        style={active
                      ...
                      >
                        <span className="text-base leading-none">{mg.emoji}</span>
                        <span className="truncate">{mg.label}</span>
                        {active && <CheckCircle2 size={13} className="ml-auto flex-shrink-0" style={{color:'var(--accent)'}} />}
                      </button>
                    );
                  })}
                </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <FileText size={15} style={{color:'var(--n-500)'}} />
                  <span className="text-sm font-bold" style={{color:'var(--n-900)'}}>Observações da sessão</span>
                </div>
                <textarea
                  value={modalNotes}
                  onChange={e => setModalNotes(e.target.value)}
                  placeholder="Ex: Aumentei carga no supino, aluno relatou dor no joelho..."
                  rows={3}
              ...
              {/* Delete + Save */}
              {showDeleteConfirm ? (
                <div className="rounded-xl p-4" style={{background:'var(--error-light)',border:'1px solid var(--error)'}}>
                  <p className="text-sm font-bold mb-3" style={{color:'var(--error)'}}>Remover este agendamento?</p>
                  <div className="flex gap-2">
                    <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-2 rounded-lg text-sm font-semibold" style={{border:'1px solid var(--n-200)',color:'var(--n-600)',background:'var(--n-0)'}}>Cancelar</button>
                    <button onClick={handleDeleteApt} className="flex-1 py-2 rounded-lg text-sm font-bold text-white" style={{background:'var(--error)'}}>Sim, remover</button>
                  </div>
                </div>
              ) : (
                <div className="flex gap-2 pb-2">
                  <button
                    onClick={() => { haptic(8); setShowDeleteConfirm(true); }}
                    className="p-2.5 rounded-xl transition-all touch-manipulation active:scale-90"
                    style={{border:'1px solid var(--n-200)',color:'var(--error)',background:'var(--error-light)'}}
                  ...
                  <button
                    ...
                    style={{border:'1px solid var(--n-200)',color:'var(--n-600)',background:'var(--n-0)'}}
                  >
                    Cancelar
                  </button>
                  <button
